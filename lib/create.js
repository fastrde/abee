...
var removeFirstFiles = function(appName){
  ...
  h.print ("'"+appName+"/"+tplDir+"' should be a good place to look for templates (and place yours later)\n", "important");
 ...
 */
var loadDirs = function(configName){
  var dirs = {};
  if (h.isFile(__dirname + "/../configs/" + configName + ".js")){
    config.load(configName);
    dirs[global.generatedAppName] = config.get('structure');
  }else{
    h.print("config " + configName + " not found", "error");
    return;
  }
  return dirs;   
};
  ...
  run: function(appName, configName){ 
    ...
    exec('meteor create ' + appName, function(error, stdout, stderr) {
      ...
      try{
        ...
        config.init(appName, configName);
      }catch(e){
        //console.log(e);
      }
    });
