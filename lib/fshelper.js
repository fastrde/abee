...
module.exports = {
  ...
  addToFile: function(file, snippet, mark){
    try{
      var fileContent = fs.readFileSync(file).toString();
      if (!new RegExp("^(" + mark + ")$", "m").test(fileContent)){
        h.print("Please insert mark " + mark + " in " + file + ". I don't know how to add the snippet.", "error");
        process.exit(1);
      }
  ...
  /**
   * return homedirectory (platform-independent)
   */
